...
- 短線研究分：大盤與自選股廣度、5/20 日動能、均線、RSI、ATR、60 日突破、量能、相對大盤、類股強弱、法人動向與新聞趨勢。
- 中期研究分：20/60 日趨勢、均線同向、波動度、相對大盤、類股強弱與營收/籌碼輔助。
- 中長線研究分：60 日趨勢、均線、波動度、回撤、類股強弱、EPS/ROE、月營收年增/月增、估值與籌碼風險。
- 新聞會抓最近多則 Google News RSS 標題，依新鮮度、利多/利空關鍵字、來源數量與新舊趨勢計分；新聞仍是低權重輔助，抓不到時採中性，不把缺資料當利空。
- 流程會先快速初篩全部 Watchlist，再對短／中／長各前五名的聯集做深入研究；`Research` 會標示「初篩」或「深入」。
- 報告另提供「短長線共識」與「資料可信度」，避免只因單一指標或缺資料就給出過度肯定的結論。
